...
1. Зайти в VSC и открыть папку, в которой необходимо создать репозиторий
2. Открыть терминал, используя сочетание клавиш Ctrl + Ё или через вкладку сверху
3. Проверить работоспособность гита командой:
```Bash
git --vresion
```
4. Написать в терминале команду для создания репозитория:
```Bash
git init
```
